...
For road-quality outcomes, we apply the same close-election RD logic in a stacked-event setting. Each renewal election defines an event, and predicted road quality is aggregated over pre-election and post-election windows around that vote. We omit election-year imagery ($t+0$) and censor each event window at the adjacent renewal elections so that the same subdivision-year is not assigned to multiple levy cycles. The main road-quality table fixes the pre-election window at $t-3$ to $t-1$ and reports rolling 3-year post-election windows, controlling for the corresponding pre-election level. Table \ref{tab:roadquality_estimates} reports NAIP-based raw Road Quality Rating and Road Quality Score outcomes so the units remain directly interpretable across columns. Supporting outputs report pooled standardized NAIP-OSIP3 outcomes, placebo tests, and alternative event-window specifications.
