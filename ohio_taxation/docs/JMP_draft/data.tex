...
Surface roads in Ohio last 15-20 years and deteriorate faster with poor drainage, utility cuts to the road, and snowplow damage \citep{hudson2020faq}. 
...
\begin{table}[H]
\centering
\caption{Predicted Road Quality Ratings by Treatment Status and Period}
\label{tab:road_quality_panel}
\begin{tabular}{lcc}
\toprule
\textbf{Period} & \textbf{Control (Renewed Levy)} & \textbf{Treated (Failed Levy)} \\
\midrule
Pre-Election  & 1.00 & 1.38  \\
 & (0.913) & (0.770) \\
Post-Election & 1.07 & 0.93 \\
 & (0.884) & (0.753) \\
\bottomrule
\end{tabular}
\vspace{0.5em}
...
\footnotesize
\textit{Notes:} This table presents the average predicted road quality ratings for townships before and after road tax levy elections. The control group consists of townships within the effective RD bandwidth that successfully renewed their road tax levy; whereas the treated group includes ones that failed to renew. Ratings are derived from our fine-tuned Vision Transformer model, where higher values indicate better road conditions. Standard deviations are reported in parentheses below the means.
\end{minipage}
\end{table}

Table \ref{tab:road_quality_panel} reports average predicted road quality ratings for townships before and after their local road tax levy elections, disaggregated by treatment status. The treated group consists of townships within the effective RD bandwidth that narrowly failed to renew their road tax levies, while the control group includes townships that narrowly passed them. Road quality is assessed using our fine-tuned Vision Transformer model (GPT-4o), applied to high-resolution satellite images obtained from Google Earth Pro. The model classifies each road segment into one of three categories: poor (0), decent (1), or high quality (2), and assigns each image a predicted class label accordingly.

Prior to the election, treated townships—those that eventually failed to renew their road tax levies—displayed higher average road quality ratings (1.38) compared to the control group (1.00). However, this relative advantage disappears in the post-election period: treated townships experience a significant decline in predicted road quality, dropping to an average rating of 0.93, while control townships exhibit a slight improvement to 1.07. This reversal suggests that the failure to renew road funding may have led to measurable deterioration in road conditions, consistent with the hypothesized effect of underinvestment in local infrastructure. Further, standard deviations in Table \ref{tab:road_quality_panel} indicate high within-group variation in road conditions, which may stem from unobserved heterogeneity across the groups and measurement noise in satellite predictions. To address this, we implement a difference-in-differences (DiD) analysis in Section \ref{sec:road_quality_decline}, which controls for time-invariant factors and common shocks, allowing for a cleaner estimate of the effect of tax cuts on road quality.
