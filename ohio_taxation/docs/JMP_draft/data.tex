...
\noindent 
where $\hat{r}$ is the predicted road quality rating, $\hat{c}$ is the confidence score, $w$ is a relative weight for allotted size of each road quality rating group, 0, 1, and 2. $\tau$ is a parameter for confidence weights\footnote{We set this equal to 0.4.}. The RQS ranges from 1 to 100, where 0 indicates poor quality and 100 indicates high quality. We use this RQS in our analysis to assess the impact of cutting local road taxes on road quality.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        Pre‐Election  & 1.47 & 1.08 \\
                      & (0.717) & (0.900) \\
        Post‐Election & 0.842 & 1.00 \\
                      & (0.688) & (0.894) \\
        \addlinespace
        \multicolumn{3}{l}{\textbf{Panel B: Road Quality Score (1–100)}} \\
        Pre‐Election  & 64.5 & 52.1 \\
                      & (22.0) & (26.3) \\
        Post‐Election & 49.4 & 49.3 \\
                      & (21.6) & (26.0) \\
        \bottomrule
    \end{tabular}
    \vspace{0.5em}
...
%         \addlinespace
%         \multicolumn{3}{l}{\textbf{Panel B: Road Quality Score (1–100)}} \\
%         Pre-Election  & 62.1 & 50.5 \\
%                       & (24.6) & (29.3) \\
%         Post-Election & 47.9 & 52.1 \\
%                       & (23.6) & (28.4) \\
%         \bottomrule
%     \end{tabular}
%     \vspace{0.5em}
%     \begin{tablenotes}
%         \footnotesize
%         \item \textit{Notes:} This table reports the average predicted road quality rating and score for townships before and after road tax levy elections. The control group includes townships within the effective RD bandwidth that successfully renewed their road tax levies; the treated group includes those that failed to renew. Ratings are derived from our fine-tuned Vision Transformer model, where higher values reflect better road conditions. Scores are based on Equation~\ref{eq:rqs_formula}. Standard deviations are shown in parentheses.
%     \end{tablenotes}
%     \end{minipage}
%     \end{threeparttable}
% \end{table}
...
% \noindent
Panel A of Table \ref{tab:road_quality_panel} summarizes the Road Quality Rating (RQR), showing that treated areas (failed levy) started with a higher average RQR (1.47) than control areas (1.08) before the referendum, but experienced a notable decline from 1.47 to 0.842 after the referendum, while control areas saw effectively no change and moved negligibly from 1.08 to 1.00. 

% \noindent
Panel B of Table \ref{tab:road_quality_panel} presents the continuous Road Quality Score (RQS), where treated areas also began with a higher average RQS ($64.5$) compared to control areas ($52.1$) before the referendum. After the referendum, RQS dropped notably from $64.5$ to $49.4$ in treated cities, but only slightly from $52.1$ to $49.3$ in control cities. This pattern suggests that failing to renew a road tax levy leads to a decline in both RQR and RQS, while renewing the levy is associated with fairly stable road quality. Section \ref{sec:road_quality_decline} provides a more detailed analysis of the impact of cutting road taxes on road quality, using the predictions from ViT model as the outcome variable.
...
A vote share of 50\% or more against the renewal road tax levy means the levy fails and the tax will no longer be collected, resulting in a stoppage of road funding via that particular tax levy. There are 3,184 referendum results in our sample, 83\% of which renew the tax, and 17\% of which cut taxes and road maintenance. The Great Financial Crisis falls in the middle of our dataset, so readers might wonder if voting behavior was affected, but we find vote share the same to two decimal points during and outside the years 2008-2009. 
