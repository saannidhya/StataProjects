capture log close
log using "C:\Documents and Settings\Steph Cellini\My Documents\Research\BondTest\Stata\test_makerecursivepanel.log", replace
...
**** This program alters the "makeshortpanel" program to create a shortpanel from -2 to +15 instead of -2 to 6;
**** The data is the used in the recursivereg.v1_test.do program;

**** This program updates Jesse's program to run the static panel analysis on the test score data;
**** Steph Cellini;
**** Updated July 1, 2008;
...
*** Note that this is the only place where the program is different than the makeshortpanel program;

** Used this command in first version of the program, but now I keep all years;
 *keep if dyear>=-2 & dyear<=15;
...
save "Data/test_recursivepanel.dta", replace;
