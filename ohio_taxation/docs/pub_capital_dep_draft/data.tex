...
Public road capital depreciates at an annual rate of approximately three percent according to U.S. Bureau of Economic Analysis fixed-asset accounts, and the cumulative under-investment in U.S. infrastructure is estimated at \$2.6 trillion through 2029 \citep{asce2025report}. Two features of this depreciation process are central to the framing of this paper. First, depreciation is gradual: physical decay accumulates over multiple years before becoming visually apparent. Second, residents and prospective home-buyers observe physical road decay {\it directly}, but they observe the underlying maintenance budget only indirectly through ballot questions and intermediate news reports. The output of the fine-tuned vision model introduced below is best understood, accordingly, not merely as a measurement instrument but as a measure of the {\it observable signal} that drives the belief-updating mechanism in Section \ref{sec:belief_updating}.
...
{\bf Ohio Satellite Images.} While the fine-tuning of our vision image-classification model uses images from \cite{brewer2021}, our Ohio application uses aerial imagery from the National Agriculture Imagery Program (NAIP) and Ohio Statewide Imagery Program (OSIP3). These sources provide substantially broader coverage than the earlier hand-collected sample, but image resolution and timing still vary across products and years. For that reason, our empirical analysis does not rely on a single before-and-after image pair. Instead, Section \ref{sec:road_quality_decline} aggregates predicted road quality within pre-election and post-election windows around each renewal election, excludes election-year imagery, and censors each window at adjacent renewal elections so the same subdivision-year is not reused across levy cycles. For interpretability, the main Table \ref{tab:roadquality_estimates} reports NAIP-based Road Quality Rating and Road Quality Score outcomes, while pooled NAIP-OSIP3 standardized estimates are used only in supporting diagnostics. Figure \ref{fig:road_quality_examples} presents two illustrative NAIP road images from Ohio, one classified as poor quality and one classified as high quality.
...
\textit{Road Quality Score.} Even though our original model output is a road quality rating\footnote{This is a direct result of our training data from \cite{brewer2021} which uses the discrete 0 (low), 1 (medium), 2 (high) road-quality metric.}, we also convert the discrete model output to a continuous measure of road quality between 1 and 100. This Road Quality Score (RQS) combines the predicted road quality rating with the model's confidence so that higher values correspond to better predicted pavement condition. We use RQS as a companion outcome to RQR because it preserves the ordinal classification while allowing finer comparisons across images and election windows.
...
\begin{table}[H]
    ...
    \begin{tablenotes}[flushleft]
        \footnotesize
        \item \textit{Notes:} The control group comprises townships within the effective RD bandwidth that successfully renewed their road tax levies; the treated group comprises those that failed to renew. Ratings are derived from our fine-tuned vision model. Scores combine the predicted class and model confidence into a 1--100 measure. Standard deviations are in parentheses.
    \end{tablenotes}
    \end{minipage}
    \end{threeparttable}
\end{table}
...
% \noindent
Panel A of Table \ref{tab:road_quality_panel} summarizes the Road Quality Rating (RQR) in a simple descriptive comparison. Treated areas, i.e. jurisdictions that cut their renewal road tax levies, started with a slightly higher average RQR (1.62) than control areas (1.56) before the referendum, but experienced a decline from 1.62 to 1.47 after the referendum, while control areas remained at 1.56. 

% \noindent
Panel B of Table \ref{tab:road_quality_panel} presents the continuous Road Quality Score (RQS), where treated areas also began with a higher average RQS (74) compared to control areas (71) before the referendum. After the referendum, RQS fell from 74 to 68 in treated cities, but only slightly from 71 to 70 in control cities. These descriptive differences are suggestive, but the formal evidence in Section \ref{sec:road_quality_decline} comes from the stacked-event regression discontinuity design.
...
Figure \ref{fig:combined} provides descriptive evidence on road quality around the referendum. Panel A illustrates the before-and-after difference in road quality rating for treated and control groups, while Panel B plots post-election road quality against vote share against renewal. Both panels are consistent with lower road quality on the fail-levy side of the cutoff, but Section \ref{sec:road_quality_decline} provides the formal stacked-event RD estimates.
...
\begin{table}[htbp]
\centering
\caption{Variable Means by Road Tax Levy Renewal Status}
\label{tab:variable_means_sd}
\small
\renewcommand{\arraystretch}{1.05}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.15cm}>{\centering\arraybackslash}p{1.45cm}>{\centering\arraybackslash}p{1.45cm}>{\centering\arraybackslash}p{1.25cm}>{\centering\arraybackslash}p{1.85cm}>{\centering\arraybackslash}p{1.85cm}@{}}
    \toprule
...
\ContinuedFloat
\centering
\caption[]{Variable Means by Road Tax Levy Renewal Status (continued)}
\small
\renewcommand{\arraystretch}{1.05}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.15cm}>{\centering\arraybackslash}p{1.45cm}>{\centering\arraybackslash}p{1.45cm}>{\centering\arraybackslash}p{1.25cm}>{\centering\arraybackslash}p{1.85cm}>{\centering\arraybackslash}p{1.85cm}@{}}
