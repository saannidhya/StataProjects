...
program define altrdbwselect, eclass
	...
	* Variances for all CCT estimators
	...
	invGamma_pilot_lq  = cholinv(Gamma_pilot_lq);	invGamma_pilot_rq  = cholinv(Gamma_pilot_rq)
	invGamma_pilot_lp  = cholinv(Gamma_pilot_lp);	invGamma_pilot_rp  = cholinv(Gamma_pilot_rp)
	...
	* Denominator
	...
	m4_l_pilot_CCT = (cholinv(quadcross(X_lq2,X_lq2))*quadcross(X_lq2,Y_l))[`q3',1]
	m4_r_pilot_CCT = (cholinv(quadcross(X_rq2,X_rq2))*quadcross(X_rq2,Y_r))[`q3',1]
	...
	* Denominator
	...
	m3_l_CCT = (cholinv(quadcross(X_lq1, w_q_l, X_lq1))*quadcross(X_lq1, w_q_l, Y_l))[q2,1]
	m3_r_CCT = (cholinv(quadcross(X_rq1, w_q_r, X_rq1))*quadcross(X_rq1, w_q_r, Y_r))[q2,1]
	...
	* Denominator
	...
	m2_l_CCT = (cholinv(quadcross(X_lq, w_b_l, X_lq))*quadcross(X_lq, w_b_l, Y_l))[`p2',1]
	m2_r_CCT = (cholinv(quadcross(X_rq, w_b_r, X_rq))*quadcross(X_rq, w_b_r, Y_r))[`p2',1]
	...
	* Add cutoff dummy interacted with v^`deriv'
	...
	*********************************************************************
	...
	y_CV_hat_l = (cholinv(quadcross(XX_CV_l,w_CV_l,XX_CV_l))*quadcross(XX_CV_l,w_CV_l,y_CV_l))[1]
	...
	for (k=0; k<=n_CV_r; k++) {
		...
		y_CV_hat_r = (cholinv(quadcross(XX_CV_r,w_CV_r,XX_CV_r))*quadcross(XX_CV_r,w_CV_r,y_CV_r))[1]
