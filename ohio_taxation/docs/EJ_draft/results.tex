...
Table \ref{tab:roadquality_estimates} reports RD estimates of the discontinuity in road quality at the 50\% vote-share threshold. Columns (1) and (2) use post-election satellite imagery and show the effect on the Road Quality Rating (RQR, 0--2) and the Road Quality Score (RQS, 1--100), respectively. Both outcomes are derived from the fine-tuned ConvNeXt v2 vision model applied to NAIP satellite imagery, as described in Section \ref{sec:road_quality}. Bandwidths are selected using the MSE-optimal procedure of \cite{calonico2019regression}, and standard errors are bias-corrected and robust, clustered by ten-digit FIPS code.

The RD estimate in Column (1) is $-0.066$, statistically significant at the 1\% level ($p < 0.001$), indicating that subdivisions narrowly voting to cut road maintenance taxes experience lower road quality than those narrowly voting to renew. The estimate in Column (2) shows a corresponding decline of $2.3$ points on the 100-point RQS scale. Using the pre-election treated-group means of $1.454$ (RQR) and $60.3$ (RQS), these estimates correspond to a decline of approximately $4.5\%$ and $3.8\%$ in road quality. The MSE-optimal bandwidth is approximately 7.8 percentage points on each side of the cutoff, yielding 875 and 358 effective observations to the left and right of the threshold, respectively.

Columns (3) and (4) report a placebo test using pre-election satellite imagery---the same RD specification applied to road quality \textit{before} the referendum takes place. If our estimates reflect a causal effect of the tax cut rather than pre-existing differences, we should find no discontinuity in pre-election road quality. Indeed, the placebo estimates are small and statistically insignificant at the 5\% level ($p = 0.076$ and $p = 0.300$ for RQR and RQS, respectively), supporting the causal interpretation of the post-election decline. Table \ref{tab:roadquality_bw_sensitivity} in the Appendix shows that the post-election RD estimate is robust across a range of bandwidths from $0.75h$ to $2h$, with point estimates remaining negative and statistically significant at the 1\% level throughout.
