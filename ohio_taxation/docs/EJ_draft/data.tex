...
\noindent 
where $\hat{r}$ is the predicted road quality rating, $\hat{c}$ is the confidence score, $w$ is a relative weight for allotted size of each road quality rating group, 0, 1, and 2. $\tau$ is a parameter for confidence weights\footnote{We set this equal to 0.8 in our main results.}. The RQS ranges from 1 to 100, where 1 indicates poor quality and 100 indicates high quality. We use this RQS in our analysis to assess the impact of cutting local road taxes on road quality.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        Pre‐Election  & 1.625 & 1.557 \\
                      & (0.4902) & (0.4986) \\
        Post‐Election & 1.467 & 1.556 \\
                      & (0.5045) & (0.4987) \\
        \addlinespace
...
% \noindent
Panel A of Table \ref{tab:road_quality_panel} summarizes the Road Quality Rating (RQR), showing that treated areas i.e. jurisdictions that cut their renewal road tax levies, started with a slightly higher average RQR (1.625) than control areas (1.557) before the referendum, but experienced a notable decline from 1.625 to 1.467 after the referendum, while control areas remained relatively stable, changing only slightly from 1.557 to 1.556. 

% \noindent
Panel B of Table \ref{tab:road_quality_panel} presents the continuous Road Quality Score (RQS), where treated areas also began with a higher average RQS (69) compared to control areas (67) before the referendum. After the referendum, RQS notably dropped 5 points from 69 to 64 in treated cities, but only slightly from 67 to 66 in control cities. This pattern suggests that failing to renew a road tax levy may lead to decline in both RQR and RQS, while renewing the levy is associated with fairly stable road quality. However, these summary statistics do not have a causal interpretation as they do not control for other factors that may influence road quality over time\footnote{We control for such factors in our analysis presented in Section \ref{sec:road_quality_decline}.}.
