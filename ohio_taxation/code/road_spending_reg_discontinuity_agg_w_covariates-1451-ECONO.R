...
covs_final$housing_roads_census_t_plus_2_matches <- covs_final$housing_roads_census_t_plus_2_matches[!(covs_final$housing_roads_census_t_plus_2_matches %in% c("pctnokids","pctmin"))]
...
covs_final$housing_roads_census_t_plus_5_matches <- covs_final$housing_roads_census_t_plus_5_matches[!(covs_final$housing_roads_census_t_plus_5_matches %in% c("pctown", "pctmin", "pctnokids"))]
...
tes_gs %>% pull(robust_coef) %>% mean / 161000

#==============================================#
# Adding Time Fixed Effects
#==============================================#

dfs_agg_covs_w_tfe <- map(dfs_agg_covs, ~ fastDummies::dummy_cols(.x, select_columns = c("year"), remove_first_dummy = TRUE) %>% relocate(starts_with("year_"), .after = "year"))
...
gs_reg <- purrr::map2(covs_final_w_tfe, dfs_agg_covs_w_tfe, .f = function(x,y){
  rdrobust(  y = y$median_sale_amount,
             ...
             covs = y %>%
               select(x) ,
             all = TRUE, kernel = "uniform", bwselect = "mserd", p = 1, q = 2, cluster = y$tendigit_fips)
})
...
y <- dfs_agg_covs_w_tfe$housing_roads_census_t_plus_2_matches
x <- covs_final_w_tfe$housing_roads_census_t_plus_2_matches[!covs_final_w_tfe$housing_roads_census_t_plus_2_matches %in% c("pctmin")]
...
rdrobust(  y = y$median_sale_amount,
           ...
           covs = y %>%
             select(x) ,
           all = TRUE, kernel = "uniform", bwselect = "mserd", p = 1, q = 2, cluster = y$tendigit_fips) %>% summary
...
covs_final_gr <- purrr::map(mgd_grwth_ws, ~find_covs(.x, y = "yoy_growth_median_sale", covs_list = covs_list))
