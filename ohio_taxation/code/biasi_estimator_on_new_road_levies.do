...
gen x = _n-6
replace x = . if x > 10
eststo nume_linear: reghdfe ln_num_employed $model, a($FE) cl(tendigit_fips) 
...
twoway 	(connected coef2 x, mcolor("$ylb") lcolor("$ylb") lw(thick) lp(solid))  (rcap up2 down2 x, lcolor("$ylb % 30")) ///
		...
		legend(ring(0) pos(11) order(1 "Effect per year" 3 "Cumulative effect")) xlabel(-5(1)10) xline(0.3, lcolor(red)) ylabel(-1(0.1)1, axis(1)) ylabel(-1(0.1)1, axis(2)) xsize(7) yline(0, lcolor(gs10) lw(vthin))
