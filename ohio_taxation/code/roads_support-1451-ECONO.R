...
#==========================================================================================================#
# Q. What is the mean housing price of houses in areas where we have these close elections
#==========================================================================================================#


bwd <- purrr::map_dbl(gs[4:length(gs)], ~  round(.x$bws[1],1)) %>% mean

tgf_list <- roads_and_census2 %>% filter(between(votes_pct_against, cutoff - bwd, cutoff + bwd)) %>% pull(tendigit_fips) %>%
  unique %>% sort

hs2 %>% filter(TENDIGIT_FIPS %in% tgf_list) %>% pull(SALE_AMOUNT) %>% mean
