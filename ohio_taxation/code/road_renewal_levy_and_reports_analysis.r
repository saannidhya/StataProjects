...
roads_r <- haven::read_dta("data/roads_and_census.dta") %>% 
  mutate(if_fail = if_else(votes_pct_against >= 50, 1, 0),
         margin = (votes_pct_against - 50)) %>% 
  janitor::clean_names() %>%
  select(tendigit_fips, year, subdivisionname, subdivisiontype, county, votes_pct_for, margin, taxtype, purpose2, description, millagepercent, duration, votes_pct_against, if_fail) %>%
  filter(toupper(subdivisiontype) == "TOWNSHIP" & description %in% c("R")) %>%
  filter(year >= 2000)
...
events2 <- roads_r %>%
  filter(toupper(subdivisiontype) == "TOWNSHIP", description %in% c("R")) %>% # keep township renewal/“R” elections only
...
m_tax = feols(dln_tax ~ fail + ln_pre_tax, data = ev_tax_did, vcov = ~fips)
m_pw  = feols(dln_pw  ~ fail + ln_pre_pw,  data = ev_pw_did,  vcov = ~fips)
...
rq_tax <- rq(dln_tax ~ fail + ln_pre_tax, data = ev_tax_did, tau = 0.5) # quantile regression on median quantile tau = 0.5
rq_pw  <- rq(dln_pw  ~ fail + ln_pre_pw,  data = ev_pw_did,  tau = 0.5)
summary(rq_tax)
summary(rq_pw)
...
boot_beta <- replicate(B, {
  ...
  coef(rq(dln_tax ~ fail + ln_pre_tax, data = ev_tax_did[idx,], tau = 0.5))["fail"]
})
quantile(boot_beta, c(.025,.5,.975))

cl_pw <- unique(ev_pw_did$fips)
B  <- 1000
boot_beta_pw <- replicate(B, {
  samp <- sample(cl_pw, length(cl_pw), replace = TRUE)
  idx  <- ev_pw_did$fips %in% samp
  coef(rq(dln_pw ~ fail + ln_pre_pw, data = ev_pw_did[idx,], tau = 0.5))["fail"]
})
quantile(boot_beta_pw, c(.025,.5,.975))
