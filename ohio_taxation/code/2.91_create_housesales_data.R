...
# Note: The Logic, "anything that is not a township" is classified as a city/village, is NOT CORRECT logic because city/village and township boundaries can overlap, and we need to preserve the city/village jurisdiction as the superseding jurisdiction in those cases. 

housesales_9515 <- housesales_9515 %>%
                    mutate(FIPS_ID = if_else(str_detect(NAMELSAD10, regex("city|village", ignore_case = TRUE)) & !str_detect(NAMELSAD10, regex("CDP", ignore_case = TRUE)),
